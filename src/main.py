...
def export_all_originals_zip():
    ...
    with db() as con:
        rows = con.execute(
            "SELECT id, original_filename, file_path FROM documents ORDER BY created_at ASC"
        ).fetchall()
    ...
    with zipfile.ZipFile(buf, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
        # Add a manifest text file listing all documents
        manifest_lines = [
    ...
    return Response(
        ...
        headers={"Content-Disposition": "attachment; filename=taxclaw-originals.zip"},
    )
